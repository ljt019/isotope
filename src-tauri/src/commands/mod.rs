...
pub async fn chat(app_handle: tauri::AppHandle, message: String) -> Result<(), String> {
    ...
    let model_manager = models::model_manager::ModelManager::init(app_handle.clone())
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let model = match models::model::Model::new(model_name.as_deref()).await {
            Ok(m) => m,
            Err(e) => {
...
pub async fn set_model(app_handle: tauri::AppHandle, model_name: String) -> Result<(), String> {
    let mut model_manager = models::model_manager::ModelManager::init(app_handle.clone())
        .map_err(|e| format!("Failed to initialize model manager: {}", e))?;

    model_manager
        .set_selected_model(
            models::model_manager::ModelOptions::from_model_name(&model_name)
                .expect("Invalid model name"),
        )
...
pub fn get_model_options() -> Result<Vec<String>, String> {
    Ok(models::model_manager::ModelOptions::all_model_names())
}
